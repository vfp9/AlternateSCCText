...
#ELIF VERSION(3) == "86"	&& Simplified Chinese
	#DEFINE ERR_UNSUPPORTEDVERSION_LOC       "SccTextX 仅支持 VFP9!"
	#DEFINE ERRORMESSAGE_LOC                 "Error #" + TRANSFORM(m.nError) + " in " + m.cMethod + " (" + TRANSFORM(m.nLine) + "): " + m.cMessage
	#DEFINE ERRORTITLE_LOC                   "程序错误"
	#DEFINE ERR_ALERTCONTINUE_LOC            "是否继续?"
	#DEFINE ERR_BADVERSION_LOC               "错误的SCCTEXTX版本。"
	#DEFINE ERR_BIN2TEXTNOTSUPPORTED_LOC     "'&cType' 类型文件不支持生成文本文件。"
	#DEFINE ERR_FCREATE_LOC                  "FCREATE() 错误： "
	#DEFINE ERR_FIELDLISTTOOLONG_LOC         "字段列表太长。"
	#DEFINE ERR_FILENOTFOUND_LOC             "文件没有发现： "
	#DEFINE ERR_FOPEN_LOC                    "FOPEN() 错误： "
	#DEFINE ERR_FOXERROR_11_LOC              "函数参数值，类型或计数无效。"
	#DEFINE ERR_INVALIDREVERSE_LOC           "参数 REVERSE 无效。"
	#DEFINE ERR_INVALIDTEXTNAME_LOC          "参数 TEXTNAME 无效。"
	#DEFINE ERR_LINENOACTION_LOC             "没有采取任何行动： "
	#DEFINE ERR_MAXBINLEN_LOC                "MAXBINLEN 的值必须是 8 的倍数。程序中止。"
	#DEFINE ERR_NOTABLE_LOC                  "需要表名。"
	#DEFINE ERR_NOTEXTFILE_LOC               "创建表需要文本文件名。"
	#DEFINE ERR_OVERWRITEREADONLY_LOC        "文件 &cParameter1 是只读的。是否覆盖？"
	#DEFINE ERR_TEXT2BINNOTSUPPORTED_LOC     "'&cType' 类型文件不支持生成二进制文件。"
	#DEFINE ERR_UNSUPPORTEDFIELDTYPE_LOC     "不支持的字段类型： "
	#DEFINE ERR_UNSUPPORTEDFILETYPE_LOC      "不支持的文件类型： "
	* Used by the thermometer
	#DEFINE C_BINARYCONVERSION_LOC	        "正在转换二进制数据：&cBinaryProgress.%"
	#DEFINE C_THERMCOMPLETE_LOC		        "&cThermLabel 生成完成"
	#DEFINE C_THERMLABEL_LOC		           "正在生成 &cThermLabel"
